...
- (BOOL)createDB {
    ...
    if ([filemgr fileExistsAtPath:databasePath] == NO) {
        ...
        if (sqlite3_open(dbpath, &database) == SQLITE_OK) {
            char *errMsg;
            const char *sql_stmt = "create table if not exists events (regno integer primary key, content text, department text, year text)";
...
- (BOOL)saveData:(NSString *)registerNumber name:(NSString *)name department:(NSString *)department year:(NSString *)year {
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK) {
        ...
        studentsDetail(regno, name, department, year)
        values
